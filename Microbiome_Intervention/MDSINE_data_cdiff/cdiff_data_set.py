...
class CdiffDataLoader(TimeSerieDataLoader):
    ...
    def _read_file(self, title, bactria_as_feature_file, samples_data_file, created_data):
        ...
        for i, row in enumerate(preproccessed_data.values):
            id_to_features_map[int(preproccessed_data.index[i])] = row
...
if __name__ == "__main__":
    ...
    create_data = True
    if create_data:
        cdiff_dataset = CdiffDataLoader(title=task, bactria_as_feature_file=bactria_as_feature_file,
                                             samples_data_file=samples_data_file,  taxnomy_level=tax, created_data=True)

    calc_significant_bacteria = True
    all_times_all_bact_results_path = "all_times_all_bacteria_best_models_results_df.csv"
    # if we wnt to run on a single model, change to a different csv file with the same structure
