function [cc,rms] = computeAvgTrajectoryCorrCdiff(concentration,Y_est,T_data,T_dense,T_test,otus)
...
for sidx=1:length(otus),
    s = otus(sidx);
    cor = corr(log10(concentration{s}(T_data_idx)+0.1),log10(Y_est(T_dense_idx,s)+0.1));
    %cor = corr((concentration{s}(T_data_idx)),(Y_est(T_dense_idx,s)));
    cc = cc + cor(1,1);

    rms = [rms ; (log10(concentration{s}(T_data_idx)+0.1) - log10(Y_est(T_dense_idx,s)+0.1)).^2];
