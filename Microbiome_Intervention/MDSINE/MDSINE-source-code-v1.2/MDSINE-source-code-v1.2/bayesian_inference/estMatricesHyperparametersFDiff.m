function [otu_scale,X,F_hat_prime,growth_mean_p,growth_std_p,self_reg_mean_p,interact_std_p,deriv_std,perturb_std_p] = estMatricesHyperparametersFDiff(f,df_dt,intervene_matrix,rescale,perturbations)
...
% methods; assuming derivatives are computed via the forward difference
% method

% inputs:
% f = spline trajectory estimates
% df_dt = spline derivative estimates
...
for o=1:numOTUs,
    ...
    for i=1:numSubjects,
        ...
        %    MLVT = repmat(f{i}(no_intervene,o)/otu_scale(o),1,numOTUs);
            MLVT = f{i}(no_intervene,:)./repmat(otu_scale',length(no_intervene),1);
            if ~isempty(perturbations),
                perturbMask = perturbations{i};
                MLVI = perturbMask;
                X{o} = [X{o} ; [ones(size(f{i}(no_intervene,o)))/otu_scale(o) MLVT MLVI]];
            else,
                X{o} = [X{o} ; [ones(size(f{i}(no_intervene,o)))/otu_scale(o) MLVT]];
            end;
