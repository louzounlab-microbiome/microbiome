function [BMD,T_BMD,sigma_biomass_est] = reformatBiomassData(BMD_raw,numSubjects,params)
...
for m=1:numSubjects,
    ...
    for t=1:length(T_BMD),
        %dt = log(D(((t-1)*numReplicates + 1):((t-1)*numReplicates + numReplicates)));
        dt = log(D(t:length(T_BMD):end));
        dm = mean(dt);
        ssd = [ssd ; (dt-dm).^2];
    end;
end;
