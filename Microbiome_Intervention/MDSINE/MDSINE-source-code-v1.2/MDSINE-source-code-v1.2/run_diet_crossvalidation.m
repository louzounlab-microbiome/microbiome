function [ ] = run_diet_crossvalidation( )
...
config = formatconfig('data_diet/crossvalidation_parameters.cfg');
...
[trials] = crossValidateDiet(config, T, perturbations, intervene, counts, ...
    species_names, BMD, experimentBlocks);

[corr_lasso,corr_select,corr_L2,rms_lasso,rms_select,rms_L2] = ...
    computeDietTrialTrajectories(T, perturbations, trials, counts, ...
    intervene,experimentBlocks);

mkdir(config.general.output_dir);
save([config.general.output_dir 'diet_crossvalidation.mat'], '-v7.3')

outf = [config.general.output_dir 'diet_crossvalidation_rmse.txt'];
fid = fopen(outf, 'w');
