...
if not use_recorded:
    ...
    x_for_deep = pickle.load(open(r'C:\Thesis\GVHD_BAR\x_for_deep.p', "rb"))
    y_for_deep = pickle.load(open(r'C:\Thesis\GVHD_BAR\y_for_deep.p', "rb"))
    x_for_deep_censored = pickle.load(open(r'C:\Thesis\GVHD_BAR\x_for_deep_censored.p', "rb"))
    y_for_deep_censored = pickle.load(open(r'C:\Thesis\GVHD_BAR\y_for_deep_censored.p', "rb"))
...
# l2=17.5+-2, dropout=0, factor=10, epochs=20, number_iterations=5, number_layers=1, neurons_per_layer=20
epochs_list = [20]#[10, 50, 100] #list(range(10,100,20)) + list(range(100,200,30))
factor_list = [10] # np.arange(0.005, 1, 0.005)
dropout_list = [0] #np.arange(0, 0.8, 0.1)
l2_lambda_list = [17.5] #np.logspace(-2, 2, 5) #  0.01, 0.1, 1, 10, 100
...
number_of_configurations = len(l2_lambda_list)*len(dropout_list)*len(factor_list)*len(number_layers_list)*len(number_neurons_per_layer_list)*len(epochs_list)
config_number=0
            ...
            for number_layers in number_layers_list:
                for number_neurons_per_layer in number_neurons_per_layer_list:
                    for epochs in epochs_list:
                        config_number += 1
                        print('\nConfiguration number: ', str(config_number), '/', str(number_of_configurations))
                        y_train_values = []
                        ...
                        for i in range(number_iterations):
                            print('\nIteration number: ', str(i + 1),'/', str(number_iterations), ' for current config')
                            ...
                            if USE_CROSS_VAL:
                                ...
                                USE_SUBJECT_ONLY_TEST_TRAIN = False
                                if USE_SUBJECT_ONLY_TEST_TRAIN:

                                    number_of_samples_in_test = 0.2 * X.shape[0]
                                    count = 0
                                    index_to_use_in_test = []
                                    data_grouped = x_for_deep.groupby('Personal_ID')

                                    group_numbers = list(data_grouped.groups.keys())
                                    random.shuffle(group_numbers)  # << shuffle before print or assignment

                                    for group_number in group_numbers:
                                        index_in_group = data_grouped.groups[group_number]
                                        count += len(index_in_group)
                                        index_to_use_in_test.append(index_in_group)
                                        if count >= number_of_samples_in_test:
                                            break
                                    index_to_use_in_test = [item for sublist in index_to_use_in_test for item in sublist]
                                    index_to_use_in_train = [x for x in X.index.values.tolist() if x not in index_to_use_in_test]
                                    X_test = X.loc[index_to_use_in_test]
                                    X_train = X.loc[index_to_use_in_train]
                                    y_test = y.loc[index_to_use_in_test]
                                    y_train = y.loc[index_to_use_in_train]
                                else:
                            ...
                            # test_model.compile_nn_model(loss=my_mse_loss, metrics=[my_mse_loss])
                        ...
                        # y_test_values = [item for sublist in y_test_values for item in sublist]
                        # y_test_predicted_values = [item for sublist in y_test_predicted_values for item in sublist]
                        # print(y_test_predicted_values)
                        # GRID_SEARCH_DIRECTORY = 'subject_test_100_processes_FALse'
                        ...
                        # np.save(
                        #     GRID_SEARCH_DIRECTORY + '/' + current_configuration_str + '/y_test_predicted_values.npy',
                        #     y_test_predicted_values)
                        # np.save(GRID_SEARCH_DIRECTORY + '/' + current_configuration_str + '/y_test_values.npy',
                        #         y_test_values)

                        y_train_values = np.array(y_train_values)
                        y_train_predicted_values = np.array(y_train_predicted_values)
                        y_test_values = np.array(y_test_values)
                        y_test_predicted_values = np.array(y_test_predicted_values)

                        y_train_values = y_train_values[~np.isnan(y_train_predicted_values)]
                        y_train_predicted_values = y_train_predicted_values[~np.isnan(y_train_predicted_values)]
                        y_test_values = y_test_values[~np.isnan(y_test_predicted_values)]
                        y_test_predicted_values = y_test_predicted_values[~np.isnan(y_test_predicted_values)]

                        y_train_values = y_train_values.tolist()
                        y_train_predicted_values = y_train_predicted_values.tolist()
                        y_test_values = y_test_values.tolist()
                        y_test_predicted_values = y_test_predicted_values.tolist()
                        ...
                        if RECORD:
                            GRID_SEARCH_DIRECTORY = 'subject_test_100_processes_myloss_USE_SUBJECT_ONLY_TEST_TRAIN='+str(USE_SUBJECT_ONLY_TEST_TRAIN)
                            if not os.path.exists(GRID_SEARCH_DIRECTORY):
                                os.mkdir(GRID_SEARCH_DIRECTORY)
                            if not os.path.exists(GRID_SEARCH_DIRECTORY+'/'+current_configuration_str):
                                os.mkdir(GRID_SEARCH_DIRECTORY+'/'+current_configuration_str)
