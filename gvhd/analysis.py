...
n_components = 20
OtuMf = OtuMfHandler('saliva_samples_231018.csv', 'saliva_samples_mapping_file_231018.csv', from_QIIME=True)
preproccessed_data = preprocess_data(OtuMf.otu_file, visualize_data=True, taxnomy_level=5)
otu_after_pca_wo_taxonomy, _ = apply_pca(preproccessed_data, n_components=n_components, visualize=True)
...
for beta in betas:
    print('beta = ' + str(beta))
    K = calculate_distance(dilated_df[list(range(20))],
                           {subject_id: subject_data[list(range(20))] for subject_id, subject_data in
                            censored_data.items()}, beta)
                    ...
                    for min_child_weight in _min_child_weight:
                        for idx, reg_lambda in enumerate(_reg_lambda):
                            ...
                            for idx2, max_depth in enumerate(_max_depth):
                                ...
                                predicted_y, spearman_value, pearson_value = predict_get_spearman_value(X_train, y_train,
                                                                                                        xg_reg)
                                ...
                                spearman_train_values.append(
                                    {'mse': mse, 'spearman_rho': spearman_value['rho'], 'pearson_rho': pearson_value['rho']})
                                print(f'train results: {spearman_value, pearson_value,  mse}')

                                key = '|'.join([x + '=' + str(eval(x)) for x in
                                                ['alpha', 'n_estimators', 'min_child_weight', 'reg_lambda', 'max_depth']])
                                ...
                                predicted_y, spearman_value, pearson_value = predict_get_spearman_value(X_test, y_test, xg_reg)
    ...
    betas_values[beta] = calc_results(y_train_dict, y_test_dict, algo_name, visualize=True)
...
for beta,value in betas_values.items():
    ...
    train = value[0]
    test = value[1]
    y_train_rho.append(train['alpha=0.01|n_estimators=20|min_child_weight=20|reg_lambda=20|max_depth=5']['spearman']['rho'])
    y_test_rho.append(test['alpha=0.01|n_estimators=20|min_child_weight=20|reg_lambda=20|max_depth=5']['spearman']['rho'])

plt.figure(756)
plt.scatter(betas_as_x, y_test_rho, label='Test', linewidth=0.3)
plt.scatter(betas_as_x, y_train_rho, label='Train', linewidth=0.3)
plt.legend()
plt.title('Spearman' + r'$\rho$ vs $\beta$')
plt.xlabel(r'$\beta$')
