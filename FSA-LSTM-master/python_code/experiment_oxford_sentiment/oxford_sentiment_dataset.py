...
class OxfordSentimentDataset(Dataset):
    ...
    def _read_source(self, source):
        ...
        for sample_row in source_file:
            ...
            sample_row = sample_row.translate(str.maketrans('', '', string.punctuation)).lower().split()
            sent = sample_row[:-1]
            sentiment = int(sample_row[-1])
...
def oxford_sentiment_data_split(dataset, split_list):
    """
    ...
    for i in range(len(new_data)):
        ...
        sub_datasets.append(OxfordSentimentDataset([], ready=ready_dict))
    return sub_datasets
...
if __name__ == "__main__":
    ...
    ds = OxfordSentimentDataset(sources_)
    dl = DataLoader(ds,
                    batch_size=64,
                    collate_fn=ds.collete_fn,
                    shuffle=True)
