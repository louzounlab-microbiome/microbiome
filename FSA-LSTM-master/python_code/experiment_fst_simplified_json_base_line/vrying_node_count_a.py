...
def random_expected_modularity_frac(node_set, num_edges, partition):
    rand_gnx = dense_gnm_random_graph(len(node_set), num_edges)
    node_list = list(node_set)
    expected = len([1 for (u, v) in rand_gnx.edges() if partition[node_list[u]] == partition[node_list[v]]]) \
                                            / len(rand_gnx.edges())
    return expected
...
def simplified_json_more_edges(alphabet, states, init_state, accept_states, transitions, fst, add_edges_k):
    ...
    for i in range(left_to_add):
        ...
        if modularity_score <= targe_modularity_score:
    ...
    e = 0
    return new_transition_map,  {"cycles": len(list(simple_cycles(gnx))),
