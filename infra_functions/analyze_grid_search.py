...
def print_recursive2(object_to_print, count=0, str_to_return=''):
    spaces = count * ' '
    if type(object_to_print) is dict:
        count += 1
        for key, val in object_to_print.items():
            ...
            if key in ['rho', 'pval', 'mse']:
                ...
                try:
                    str_to_return += print_recursive2(val, count=count, str_to_return=str_to_return)
                except:
                    pass
    else:
        # spaces = (count-1) * '\t'+ ' '
...
def main():
    ...
    a=print_recursive(best_configs)
    pass
