...
def my_loss(y_true, y_pred):
    ...
    time_sense_loss = y_true[:, 2] - (y_true[:, 0] + y_pred[:, 1])
...
if PLOT_INPUT_TO_NN_STATS:
    ...
    plt.title(f'Before outlier removal - STD={std}, MED={med}, Mean={mean}')
...
if PLOT_INPUT_TO_NN_STATS:
    ...
    plt.title(f'After outlier removal - STD={std}, MED={med}, Mean={mean}')

# remove outliers


epochs_list = [10, 50, 100]#[10, 50, 100] #list(range(10,100,20)) + list(range(100,200,30))
lr_list = [0.00001] # np.arange(0.005, 1, 0.005)
dropout_list = np.arange(0, 0.8, 0.1)
l2_lambda_list = np.logspace(-3, 3, 7) # 0.001, 0.01, 0.1, 1, 10, 100, 1000
train_res, test_res = [], []
    ...
    for dropput in dropout_list:
        for lr in lr_list:
            for epochs in epochs_list:
                ...
                if RECORD:
                    GRID_SEARCH_DIRECTORY = 'GridSearch_mse_loss'
