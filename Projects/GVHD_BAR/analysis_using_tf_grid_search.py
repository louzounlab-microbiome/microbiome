...
for beta in betas:
    ...
    epochs_list = [15]  # list(range(10,100,10)) + list(range(100,1100,100))
    train_res, test_res = [], []
    for epochs in epochs_list:
        ...
        for i in range(number_iterations):
            ...
            regularizer = regularizers.l2(lambda_regularize)
            test_model.build_nn_model(hidden_layer_structure=[{'units': n_components, 'activation': tf.nn.relu, 'kernel_regularizer': regularizer},
                                                              ...
                                                              {'units': 4, 'regularization': regularizer}])
            sgd = optimizers.SGD(lr=0.01, decay=1e-6, momentum=0.9)
            test_model.compile_nn_model(loss=my_mse_loss, metrics=[my_mse_loss], optimizer=sgd)
            test_model.train_model(X_train.values, y_train.values.astype(np.float), epochs=epochs)
